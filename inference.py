...
def load_data_prompts(data_dir, video_size=(256,256), video_frames=16, interp=False):
    ...
    for entry in os.listdir(data_dir):
        input_path = os.path.join(data_dir, entry)
        try:
            assert os.path.isdir(input_path)
            print(f"The path {input_path} is a directory.")
        except AssertionError as e:
            print(e)
        file_list = os.listdir(input_path)

        if interp:
            image1 = Image.open(os.path.join(input_path, file_list[0])).convert('RGB')
            image_tensor1 = transform(image1).unsqueeze(1) # [c,1,h,w]
            image2 = Image.open(os.path.join(input_path, file_list[1])).convert('RGB')
    ...
    return filename_list, data_list
...
def image_guided_synthesis(model, prompts, videos, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if model.two_img_emb == True:
        img1 = videos[:,:,0,...]
        img2 = videos[:, :, -1, ...]
        img_emb1 = model.embedder(img1)  ## b l c
        img_emb1 = model.image_proj_model(img_emb1)
        img_emb2 = model.embedder(img2)  ## b l c
        img_emb2 = model.image_proj_model(img_emb2)
    ...
    if unconditional_guidance_scale != 1.0:
        ...
        if model.two_img_emb == True:
            uc_img_emb1 = model.embedder(torch.zeros_like(img1))  ## b l c
            uc_img_emb1 = model.image_proj_model(uc_img_emb1)
            uc_img_emb2 = model.embedder(torch.zeros_like(img2))  ## b l c
            uc_img_emb2 = model.image_proj_model(uc_img_emb2)
            uc_img_emb = torch.cat([uc_img_emb1, uc_img_emb2], dim=1)
        else:
    ...
    for _ in range(n_samples):
        ...
        if ddim_sampler is not None:
            noise = torch.randn(noise_shape, device=videos.device)
            b, c, f, h, w = noise_shape
            start_mask = noise[:, :, 0:1]
            end_mask = noise[:, :, -1:]
            se_noise = torch.cat([start_mask, end_mask], dim=2)  # 将起始和结束张量拼接
            noise = torch.nn.functional.interpolate(se_noise, size=(f, h, w), mode='trilinear', align_corners=True)
            kwargs.update({'x_T': noise})
...
def run_inference(args, gpu_num, gpu_no):
    ...
    filename_list, data_list = load_data_prompts(args.prompt_dir, video_size=(args.height, args.width), video_frames=n_frames, interp=args.interp)
    ...
    data_list_rank = data_list
    filename_list_rank = filename_list
...
def get_parser():
    ...
    parser.add_argument("--FT16", action='store_true', default=False, help="Model precision")
    return parser
