...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        # x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        try:
            inerp = self.interp_noise
        except AttributeError:
            inerp = False
        try:
            interp_new = self.interp_new
        except AttributeError:
            interp_new = False
        ...
        if inerp:
            ...
            noise = nn.functional.interpolate(se_noise, size=(f, h, w), mode='trilinear', align_corners=True)
        if interp_new:
            b, c, f, h, w = x_start.shape
            start_mask = noise[:,:,0:1]
            end_mask = noise[:,:,-1:]
            ...
            a_values = torch.linspace(0, 1, steps=num_interpolations + 2)[1:-1]  # 线性插值，排除首尾的0和1
            interpolated_tensors = []

            for a in a_values:
                interpolated_tensor = torch.sqrt(a) * end_mask + torch.sqrt(1 - a) * start_mask
                interpolated_tensors.append(interpolated_tensor)

            # 将首尾张量添加到列表
            all_tensors = [start_mask] + interpolated_tensors + [end_mask]

            # 在通道维度后新增维度，并沿新维度 f 拼接
            # all_tensors_with_new_dim = [tensor.unsqueeze(2) for tensor in all_tensors]  # 在通道维度后插入新维度
            noise = torch.cat(all_tensors, dim=2)  # 按新维度 f 拼接
        ...
        if self.coloss:
            # loss_simple += self.coloss*self.get_loss((model_output[:, :, 1:] - model_output[:, :, :-1]),
            #                                          (target[:, :, 1:] - target[:, :, :-1]), mean=False).mean([1, 2, 3, 4])
            loss_simple += self.coloss*(1-nn.functional.cosine_similarity((model_output[:, :, 1:] - model_output[:, :, :-1]).view(1, 4, 15, -1),
                                                     (target[:, :, 1:] - target[:, :, :-1]).view(1, 4, 15, -1),dim = -1).mean([1, 2]))
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, **kwargs):
        ...
        if self.two_img_emb == True:
            img1 = x[:,:,0,...]
            img2 = x[:, :, -1, ...]
            img_emb1 = self.embedder(img1)  ## b l c
            img_emb1 = self.image_proj_model(img_emb1)
            img_emb2 = self.embedder(img2)  ## b l c
            img_emb2 = self.image_proj_model(img_emb2)
