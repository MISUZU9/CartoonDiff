...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, **kwargs):
        ...
        elif l_context == 77 + t*32:
            context_text, context_img = context[:, :77, :], context[:, 77:, :]
            context_text = context_text.repeat_interleave(repeats=t, dim=0)
            context_img1, context_img2 = torch.split(context_img, [256, 256], dim=1)
            context_img1 = rearrange(context_img1, 'b (t l) c -> (b t) l c', t=t)
            context_img2 = rearrange(context_img2, 'b (t l) c -> (b t) l c', t=t)
            context_img = torch.cat([context_img1, context_img2], dim=1)
            context = torch.cat([context_text, context_img], dim=1)
        else:
