...
from dataset_frame import Anime_frame_Dataset
...
def get_parser(**parser_kwargs):
    ...
    # parser.add_argument('--devices', type=int, default=0, help="Number of devices to use")
...
if __name__ == "__main__":
    ...
    model.coloss = config.coloss
    model.interp_noise = config.interp_noise
    model.interp_new = config.interp_new
    model.two_img_emb = False
    a = 0
    if a == 0:
        print('this is new interp')
        a = 1
    ...
    dataset = Anime_frame_Dataset(**config.data)
